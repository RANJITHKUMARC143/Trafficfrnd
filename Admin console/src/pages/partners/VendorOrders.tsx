...
const VendorOrders: React.FC = () => {
  ...
  const fetchVendorOrders = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/vendors/orders/admin?vendorId=${vendorId}`, {
  ...
  const fetchOrderDetails = async (orderId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3000/api/vendors/orders/admin/${orderId}/details`, {
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
