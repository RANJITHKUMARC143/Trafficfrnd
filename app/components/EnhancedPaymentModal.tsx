...
export default function EnhancedPaymentModal({
  ...
  const handleDirectAppPayment = async (app: PaymentApp) => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.31.107:3000/api/payments/cashfree/upi/order', {
        method: 'POST',
        headers: {
  ...
  const pollPaymentStatus = () => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`http://192.168.31.107:3000/api/orders/${orderId}`, {
          headers: {
            'Authorization': `Bearer ${await getAuthToken()}`,
          },
        });
