...
const PartnersDelivery: React.FC = () => {
  ...
  const handleDelete = async (partner: DeliveryPartner) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:3000/api/delivery/${partner.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
  ...
  const handleEditSave = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:3000/api/delivery/${form.id}`, {
        method: 'PUT',
    ...
    <div className="flex items-center space-x-1">
      <Button
        ...
        onClick={async () => {
          try {
            const token = localStorage.getItem('token');
            const res = await fetch(`http://localhost:3000/api/delivery/${row.id}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json', 'Authorization': token ? `Bearer ${token}` : '' },
