...
class SocketService {
  ...
  private readonly SOCKET_URL = 'http://192.168.131.176:3000';
      ...
      if (!this.socket.connected) {
        ...
        await new Promise((resolve, reject) => {
          ...
          this.socket?.once('connect_error', (error) => {
            console.error('Socket connection error while waiting:', error);
            clearTimeout(timeout);
            reject(error);
          });
        });
  ...
  disconnect() {
    if (this.vendorId) {
      this.socket?.emit('leaveVendorRoom', this.vendorId);
    }
    this.socket?.disconnect();
    this.socket = null;
