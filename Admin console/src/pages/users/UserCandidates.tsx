...
const UserCandidates: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://trafficfrnd-2.onrender.com'}/api`, {
  ...
  const handleDelete = async (user: UserType) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://trafficfrnd-2.onrender.com'}/api/${user.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
  ...
  const handleEditSave = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://trafficfrnd-2.onrender.com'}/api/${form.id}`, {
        method: 'PUT',
