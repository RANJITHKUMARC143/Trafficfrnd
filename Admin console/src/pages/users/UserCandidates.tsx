...
const UserCandidates: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api`, {
  ...
  const handleDelete = async (user: UserType) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/${user.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': token ? `Bearer ${token}` : '' }
      });
  ...
  const handleEditSave = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/${form.id}`, {
        method: 'PUT',
